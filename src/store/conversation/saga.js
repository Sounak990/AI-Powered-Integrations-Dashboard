...
import { 
  ...
  audioDataAvailable ,
  serverConfirmedExit,
  websocketShouldClose
} from './actions';
import { submitAudioData } from '../../services/api';
...
function createWebSocketChannel(params) {
  const { username, tenantId, conversationId, scenarioId } = params;
  console.log('[Saga - createWebSocketChannel] Creating WebSocket Channel with params:', params);

  return eventChannel(emitter => {
    const websocketURL = `wss://api.dev.maraca.io/wss?username=${encodeURIComponent(username)}&tenantId=${encodeURIComponent(tenantId)}&conversationId=${encodeURIComponent(conversationId)}&scenario_id=${encodeURIComponent(scenarioId)}`;
    ...
    webSocket.onmessage = (event) => {
      ...
      if (data.type === 'server_confirmed_exit') {
        ...
        emitter(audioDataAvailable(audioBlob));
      } else if (data.type === 'audio_chunk' && data.audioData) {
        const audioBlob = base64ToBlob(data.audioData, 'audio/mp3');
        console.log('[Saga - createWebSocketChannel] Audio Chunk Available, Blob Size:', audioBlob.size);
        // Emit an action with the audio chunk
        emitter(audioDataAvailable(audioBlob));
      }
    };
...
function* handleExitConversation() {
  ...
  if (webSocket && webSocket.readyState === WebSocket.OPEN) {
    ...
    const { serverResponse } = yield race({
      serverResponse: take(actionTypes.SERVER_CONFIRMED_EXIT),
      timeout: delay(25000)
    });
    ...
    if (serverResponse) {
      ...
      yield put(websocketShouldClose());
    } else {
...
function* handleConnectWebSocket(action) {
  ...
  yield take(actionTypes.WEBSOCKET_SHOULD_CLOSE);
...
function* handleSendAudioBlob(action) {
  try {
    const data = yield call(submitAudioData, action.payload); // action.payload is the audio Blob
    console.log('Audio data sent successfully');
  } catch (error) {
    console.error('Error sending audio data:', error.message);
  }
}

export default function* conversationSaga() {
  yield takeLatest(actionTypes.CONNECT_WEBSOCKET, handleConnectWebSocket);
  yield takeLatest(actionTypes.START_RECORDING, handleStartRecording);
  yield takeLatest(actionTypes.STOP_RECORDING, handleStopRecording);
  yield takeLatest(actionTypes.EXIT_CONVERSATION, handleExitConversation);
  yield takeLatest(actionTypes.DISCONNECT_WEBSOCKET, handleDisconnectWebSocket);
  yield takeLatest(actionTypes.SEND_AUDIO_BLOB, handleSendAudioBlob);
}
